...
tfLazyResults = [
    {**z, "words": filter(lambda y: y["tf-idf"] != 0, z["words"])} for z in collection
]
...
for i in range(len(tfResults)):
    for j in range(len(tfResults[i]["words"])):
        ...
        if not currentKey in wordTotal:
            ...
            wordTotal[currentKey] = (
                tfResults[i]["words"][j]["tf-idf"] + wordTotal[currentKey]
            )

sortedWordTotal = dict(
    sorted(wordTotal.items(), key=lambda item: item[1], reverse=True)
)

pagesData = {"data": tfResults}
